---
title: Project Naming Conventions
description: Solution and project naming standards for .NET and Node.js
sidebar_position: 2
---

.NET Projects should follow the standard .NET naming conventions, which include:
- Projects: PascalCase (e.g., `CompanyName.ProjectName.TypeName`) e.g. `MyCompany.MyProject.WebApi`
- Namespaces: PascalCase (e.g., `CompanyName.ProjectName.TypeName.NamespaceName`) e.g. `MyCompany.MyProject.WebApi.Services`
- Assemblies: PascalCase (e.g., `CompanyName.ProjectName.TypeName`) e.g. `MyCompany.MyProject.WebApi.dll`

Node.js Projects should follow the standard Node.js naming conventions, which include:
- Projects: kebab-case (e.g., `project-name`) e.g. `my-project`
- Modules: camelCase (e.g., `moduleName`) e.g. `myModule`